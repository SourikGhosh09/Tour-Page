...
const uploadDir=path.resolve(process.env.UPLOAD_DIR||(process.env.VERCEL?path.join(tmpdir(),'ghure-ashi-uploads'):'uploads'));
...
app.post('/api/review-uploads',reviewUploadLimit,async(req,res)=>{if(!blobStorageEnabled)return res.status(404).json({error:{code:'BLOB_DISABLED',message:'Direct uploads are not enabled.'}});try{const response=await handleUpload({body:req.body,request:req,onBeforeGenerateToken:async(pathname,clientPayload)=>{if(!/^reviews\/[a-zA-Z0-9._-]+$/.test(pathname))throw new Error('Invalid review upload path.');let reviewFields;try{reviewFields=JSON.parse(clientPayload||'{}');}catch{throw new Error('Invalid review details.');}if(!guestReviewSchema.safeParse(reviewFields).success)throw new Error('Complete the review form before uploading media.');return{allowedContentTypes:[...allowedMedia.keys()],maximumSizeInBytes:25*1024*1024,validUntil:Date.now()+10*60_000,addRandomSuffix:true,tokenPayload:JSON.stringify({purpose:'guest-review'})};}});res.json(response);}catch(error){res.status(400).json({error:{code:'UPLOAD_TOKEN_ERROR',message:error.message||'The upload could not be authorized.'}});}});
app.post('/api/reviews',reviewLimit,upload.array('media',5),async(req,res,next)=>{
  ...
  try{
    ...
    const parsedDirectMedia=guestReviewMediaSchema.safeParse(directMedia);
    if(directMedia.length&&(!blobStorageEnabled||!parsedDirectMedia.success))return res.status(422).json({error:{code:'INVALID_MEDIA',message:'Upload up to five approved photos or videos.'}});
    const mediaRows=[];
    if(directMedia.length){
      for(const item of parsedDirectMedia.data){
...
app.post('/api/admin/logo-uploads',requireAdmin,requirePermission('settings'),async(req,res)=>{if(!blobStorageEnabled)return res.status(404).json({error:{code:'BLOB_DISABLED',message:'Direct uploads are not enabled.'}});try{const response=await handleUpload({body:req.body,request:req,onBeforeGenerateToken:async pathname=>{if(!/^logos\/[a-zA-Z0-9._-]+$/.test(pathname))throw new Error('Invalid logo upload path.');return{allowedContentTypes:[...allowedLogoMedia.keys()],maximumSizeInBytes:5*1024*1024,validUntil:Date.now()+10*60_000,addRandomSuffix:true};}});res.json(response);}catch(error){res.status(400).json({error:{code:'UPLOAD_TOKEN_ERROR',message:error.message||'The logo upload could not be authorized.'}});}});
...
app.delete('/api/admin/packages/:id',requireAdmin,requireMaster,async(req,res,next)=>{try{await transaction(client=>deletePackage(client,req.params.id,req.admin.id));res.status(204).end();}catch(e){next(e);}});
...
app.delete('/api/admin/bookings/:id',requireAdmin,requireMaster,async(req,res,next)=>{try{await transaction(client=>deleteBooking(client,req.params.id,req.admin.id));res.status(204).end();}catch(e){next(e);}});
